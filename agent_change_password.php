...
<?php
	if(isset($_POST['newpass'])!='')
	{
		//echo "sad";
		//exit;
		$rows=fetch_query("password","agent",array("id="=>$_SESSION['ntexpress_retroagent']));
		$pwd = $rows['password'];
		if($pwd!=$_POST['oldpass'])
		{
			$_SESSION['unsuc']='Invalid Old Password';
			header("location:".$site_url."agent-change-password");
			exit;
		}
		else
		{
			$arr=array("password"=>$_POST['newpass']);
			$update = update_query($arr,"id=".$_SESSION['ntexpress_retroagent'],"agent");
			if($update)
			{
		...
		//exit; 
		?>
		<script>window.location="<?php echo $site_url?>agent-change-password";</script><?php
		exit;
	}
