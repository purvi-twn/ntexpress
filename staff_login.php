...
<body class="hold-transition bg-img" style="background-image: url(images/login-img.jpeg)" data-overlay="4" cz-shortcut-listen="true">
...
if(isset($_POST['username'])!='')
{
	...
	$sel=$dlink->query("select id,fname,mname,lname,dept_branch,branch,role,username from department where username='".$_POST['username']."' and password='".$_POST['password']."'");
	...
	if($sel->num_rows>=1)
	{
		...
		if($res['role'] == 'Sales Operator' || $res['role'] == 'sales operator')
		{
			$_SESSION['ntexpress_retrosales']=$res['username'];
			$_SESSION['ntexpress_retroagent']=$res['id'];
			//echo $_SESSION['ntexpress_retrosales']; exit;
		}
		else
		{
		
			$_SESSION['ntexpress_retroagent']=$res['id'];
			$_SESSION['ntexpress_retrostaff']=$res['fname']." ".$res['mname']." ".$res['lname'];
			$_SESSION['ntexpress_retrodept_branch']=$res['dept_branch'];
			$_SESSION['ntexpress_retrobranch']=$res['branch'];
		}
	...
	else
	{
		...
		<script>window.location="<?php echo $site_url?>staff-login";</script><?php
		exit;
	}
}
