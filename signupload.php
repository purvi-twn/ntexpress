...
use Dompdf\Dompdf;
    ...
    $sender = $_POST['sendersignatureurl'];
    $receiver = $_POST['receiversignatureurl'];
    $id = $_POST['iid'];


    $sender = str_replace('data:image/png;base64,', '', $sender);
    $sender = str_replace(' ', '+', $sender);
    $sender_data = base64_decode($sender);

    $success = file_put_contents('signupload/'.$id.'_sender.png', $sender_data);
	//create jpg from png
	$image = imagecreatefrompng('signupload/'.$id.'_sender.png');
	...
	imagejpeg($bg, 'signupload/'.$id.'_sender' . ".jpg", $quality);
	imagedestroy($bg);
	

    $receiver = str_replace('data:image/png;base64,', '', $receiver);
    $receiver = str_replace(' ', '+', $receiver);
    $receiver_data = base64_decode($receiver);

    $success1 = file_put_contents('signupload/'.$id.'_receiver.png', $receiver_data);
	$image = imagecreatefrompng('signupload/'.$id.'_receiver.png');
	$bg = imagecreatetruecolor(imagesx($image), imagesy($image));
	imagefill($bg, 0, 0, imagecolorallocate($bg, 255, 255, 255));
	imagealphablending($bg, TRUE);
	imagecopy($bg, $image, 0, 0, 0, 0, imagesx($image), imagesy($image));
	imagedestroy($image);
	$quality = 50; // 0 = worst / smaller file, 100 = better / bigger file 
	imagejpeg($bg, 'signupload/'.$id.'_receiver' . ".jpg", $quality);
	imagedestroy($bg);
	@unlink('signupload/'.$id.'_sender.png');
	@unlink('signupload/'.$id.'_receiver.png');

    if($success && $success1)
    {
        $sender_sign=$id . '_sender.jpg';
        $receiver_sign=$id . '_receiver.jpg';

        $arr=array("sender_sign"=>$sender_sign,"receiver_sign"=>$receiver_sign); 
        ...
        if($insert)
        {
            ...
            $row=select_query("*","shipment", array("id="=>$id), "id asc");
            if($row->num_rows)
            {
            ...
            $_SESSION['suc']='Signature Addedd Successfully...';
        }
