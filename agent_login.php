...
{
	$sel=$dlink->query("select email,id from agent where email='".$_POST['username']."' and password='".$_POST['password']."'");
	if($sel->num_rows>=1)
	{
		$res=$sel->fetch_array();
	...
	else
	{
		...
		<script>window.location="<?php echo $site_url?>agent-login";</script><?php
		exit;
	}
}
