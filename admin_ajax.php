...
use Dompdf\Dompdf;
if($_REQUEST['action']=='get_other_invoices')
{
	global $dlink;
	$rowno=$_REQUEST['rowno'];
	//$qry="select * from shipment where 1 order by id desc LIMIT 10 OFFSET $rowno";
	$unm=""; $row=''; $num_rows=0; $qry='';
	if(@$_SESSION['ntexpress_retrostaff']!="")
	{
		$unm=$_SESSION['ntexpress_retrostaff'];
		$qry="select * from shipment where 1 and sales_person LIKE '%".$unm."%' order by id desc";
	}
	else
	{
		$qry="select * from shipment where 1 order by id desc";
	}
//	$qry="select * from shipment where 1 order by id desc";
	...
	if(mysqli_num_rows($empRecords)>0)
	{
		...
		$total_amount=0;
		$total_qnty=0;
		while($b=mysqli_fetch_assoc($empRecords))
		{
			...
			$delivery_date="";
			if($b['delivery_date']=='1970-01-01 05:30:00' || $b['delivery_date']=='1970-01-01 03:00:00')
				$delivery_date="-"; 
			else 
			...
			{
				$cod_paymentmode='checked';
			}
			$cash_paymentmode='';
			if($b['mode_of_payment']== 'Cash')  
			{
				$cash_paymentmode='checked';
			}

			$cash=0;
			$cod=0;
			if($b['mode_of_payment']=='Cash')
				$cash=$b['finaltotal_val'];
			if($b['mode_of_payment']=='COD')
				$cod=$b['finaltotal_val'];

			$mode=$b['mode_of_payment'].'<i class="fa fa-edit" style="font-size:16px" data-toggle="modal" data-target="#myModal_'.$b['id'].'" data-id="'.$b['id'].'" id="editCompany"></i>
            	...
            	<div class="modal-dialog">
                	<div class="modal-content">
                        ...
                        <form action="" method="post">
							...
							<input  type="radio" name="paymentmode" value="COD" id="COD" '.$cod_paymentmode.'> COD
                            <br>
                            <input type="radio" name="paymentmode" value="Cash" id="Cash" '.$cash_paymentmode.'> Cash
                            <br>
		...
		$status_check_inprocess='';
		$status_check_delivered='';
		$status_check_reject='';
		$status_check_return='';
		$status_check_paid='';
		if ($b['status'] == '1')
			$status_check_pending='selected="selected"';
		if ($b['status'] == '2')
			$status_check_inprocess='selected="selected"';
		if ($b['status'] == '3')
			$status_check_delivered='selected="selected"';
		if ($b['status'] == '4')
			$status_check_reject='selected="selected"';
		if ($b['status'] == '5')
			$status_check_return='selected="selected"';
		if ($b['status'] == '6')
			$status_check_paid='selected="selected"';
		$status='<select name="status'.$b['id'].'" id="status'.$b['id'].'" class="form-control" onChange="change_status("'.$b['id'].'",this.value)">
        <option '.$status_check_pending.' value="1" >Pending</option>
        <option '.$status_check_inprocess.' value="2" >In Process</option>
        <option '.$status_check_delivered.' value="3" >Delivered</option>
        <option '.$status_check_reject.' value="4" >Reject</option>
        <option '.$status_check_return.' value="5" >Return</option>
        <option '.$status_check_paid.' value="6" >Paid</option>
        </select>';
		$invoice_send="";
		if(@$_SESSION['ntexpress_retrostaff']!="")
		{
			$invoice_send='<a style="width: 80px;color: white;" onclick="capturesign('.$b['id'].')" class="btn btn-info btn-xs mb-5">Capture Sign</a><br />
			<a style="width:57px" href="'.$site_url.'_private_content_shipment/l_'.$b['invoice_number'].'.pdf" target="_blank"  class="btn btn-info btn-xs mb-5">Invoice</a><br />
		...
		$invoice_send='
		<a style="width: 80px;color: white;"  onclick="capturesign('.$b['id'].')" class="btn btn-info btn-xs mb-5">Capture Sign</a><br />
		<a style="width:57px" href="generate_invoice.php?iid='.$b['id'].'" target="_blank"  class="btn btn-info btn-xs mb-5">Create Invoice</a><br /><a style="width:57px" href="'.$site_url.'_private_content_shipment/l_'.$b['invoice_number'].'.pdf" target="_blank"  class="btn btn-info btn-xs mb-5">Invoice</a><br />
		 <a style="width: 80px;color: white;" onClick="sendemail("'.$b['id'].'")" class="btn btn-info btn-xs mb-5">Send Email</a><br />';
		}

		$invoice_send.='<div class="modal" id="signModal_'.$b['id'].'" role="dialog">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <button type="button" class="close" data-dismiss="modal">&times;</button>
                            ...
                            <div class="modal-body">
                                <form action="signupload.php" method="post">
                                    ...
                                    <input type="hidden" name="iid" value="'.$b['id'].'">
                                    <div class="form-group two-div-select">
                                        <div class="row">
                                            <div class="col-xl-6 col-lg-6 col-md-6 col-sm-6 col-12">
                                                <label for="">Sender signature</label>
                                                <canvas id="sendersignature_'.$b['id'].'" width="300" height="100" style="border: #e4e8eb;border-style: solid;"></canvas>
                                                <div id="errsendersign"></div>
                                                <input type="hidden" name="sendersignatureurl" id="sendersignatureurl_'.$b['id'].'" value="" />
                                                <input type="button" id="sendersigclear_'.$b['id'].'" value="Clear Signature" class="submit-button border-0"/>
                                            </div>
                                    ...
                                    <div class="form-group two-div-select">
                                        <div class="row">
                                            <div class="col-xl-6 col-lg-6 col-md-6 col-sm-6 col-12">
                                                ...
                                                <canvas id="receiversignature_'.$b['id'].'" width="300" height="100" style="border: #e4e8eb;border-style: solid;"></canvas>
                                                <div id="errreceiversign"></div>
                                                <input type="hidden" name="receiversignatureurl" id="receiversignatureurl_'.$b['id'].'" value="" />
                                                <input type="button" id="receiversigclear_'.$b['id'].'" value="Clear Signature" class="submit-button border-0"/>
                                            </div>
                                        </div>
                                    </div>
			...
			$data['data'][] = array(
				"sr_no"=>$no,
				"date"=>date('d-m-Y',strtotime($b['invoice_date'])),
				"branch_name"=>$dept_nm['bname'],
...
if($_REQUEST['action']=='get_report')
{
	...
	else if($_GET["report"]=='Employee')
	{
		...
		if($source_branch!=''){
			...
			$qry.=" and sales_person LIKE '%".$source_branch."%' and sales_person!=''";
		}
		if($from_date == $to_date)
			$qry.=" and date(invoice_date)='$from_date'";
		elseif($from_date!='' && $to_date!='' && $from_date!='1970-01-01' && $to_date!='1970-01-01')
	...
	else if($_GET["report"]=='Salesman')
	{
		...
		$emp_name=''; $emp_lname=''; $emp_mname='';
		if($_GET['agentid']!='');
		{
			$agent=$_GET['agentid'];
			$qry="SELECT fname,mname,lname FROM `department` WHERE 1 and id='".$agent."'";
			$emp_ = mysqli_query($dlink, $qry);
			$emp=mysqli_fetch_assoc($emp_);
		...
		elseif($from_date!='' && $to_date!='' && $from_date!='1970-01-01' && $to_date!='1970-01-01')
			$qry.=" and (invoice_date BETWEEN '$from_date' AND '$to_date')";
		if($agent!='')
			$qry.=" and (sales_person LIKE '%$emp_name%' OR sales_person LIKE '%strtolower($emp_name)%') and sales_person!=''";
		$qry.=" order by invoice_date asc";
	...
	else if($_GET["report"]=='Self')
	{
		$from_date=''; $to_date=''; $agent=''; $type ='';
		if($_GET['from_date']!='');
			$from_date=date('Y-m-d',strtotime($_GET['from_date']));
		if($_GET['to_date']!='');
			$to_date=date('Y-m-d',strtotime($_GET['to_date']));
		...
		elseif($from_date!='' && $to_date!='')
			$qry.=" and (invoice_date BETWEEN '$from_date' AND '$to_date')";
		if($agent!='')
			$qry.=" and sales_person LIKE '".$agent."'";
...
if($_REQUEST['action']=='get_customer_other_info')
{
	...
	echo $details=$records['email']."~~".$mno."~~".$records['address']."~~".$records['country']."~~".$records['state']."~~".$records['city']."~~".$records['rcvr_name']."~~".$records['rcvr_email']."~~".$records['rcvr_mno']."~~".$records['rcvr_address']."~~".$records['rcvr_country']."~~".$records['rcvr_state']."~~".$records['rcvr_city']."~~".$records['name']."~~".$records['taxtype']."~~".$records['rcvr_iqamano']."~~".$records['rcvr_vat_no']."~~".$records['customerid']."~~".$records['taxnumber']."~~".$records['area']."~~".$records['company_name'];
	exit;
...
if($_REQUEST['action']=='get_customer_other_info_for_receiver')
{
	...
	echo $records['rcvr_name']."~~".$records['rcvr_email']."~~".$records['rcvr_mno']."~~".$records['rcvr_address']."~~".$records['rcvr_country']."~~".$records['rcvr_state']."~~".$records['rcvr_city']."~~".$records['rcvr_landmark']."~~".$records['rcvr_iqamano']."~~".$records['rcvr_vat_no']."~~".$records['rcvr_company'];
	exit;
}
